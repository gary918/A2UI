...
# We don't need CORS if we serve the client from the same origin,
# but if the user wants to test with a different client we can keep it.
# For security, we should restrict it or keep it local.
# Since it's local dev, allowing localhost is good.
app.add_middleware(
    CORSMiddleware,
...
async def handle_jsonrpc(request: Request):
  ...
  if method == "message/send":
    ...
    # Extract userAction from DataPart (if any)
    user_action = None
    for part in parts:
      if (
          part.get("metadata", {}).get("mimeType")
...
if __name__ == "__main__":
  # Host MUST be 127.0.0.1 for security during local testing
